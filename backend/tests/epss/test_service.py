...
async def test_ensure_scores_exist(db_session: Session, mock_epss_client):
    ...
    mock_epss_client.get_scores.return_value = {"CVE-2023-5678": (0.2, 85.0)}
...
def test_get_scores_by_cve_ids(db_session: Session):
    score1 = CVEScore(
        cve_id="CVE-2023-1234", epss_score=0.5, last_updated=datetime.now(timezone.utc)
    )
    score2 = CVEScore(
        cve_id="CVE-2023-5678", epss_score=0.2, last_updated=datetime.now(timezone.utc)
    )
    db_session.add_all([score1, score2])
    db_session.commit()

    service = EPSSService(db_session)
    result = service.get_scores_by_cve_ids(
